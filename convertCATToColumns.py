import sys
import os
from converter import CATToColumns
...
def printUsage():
    print "usage: python convertCATToColumns.py dir_name [options]"
    print "   or: python convertCATToColumns.py file_name [options]"
    print " "
...
if __name__ == '__main__':
    ...
    if os.path.isdir(sys.argv[1]):  #input is directory name
        ...
        for r, d, f in os.walk(dirpath):
            for filename in f:
                ...
                if filename.endswith(".xml"):
                    ...
                    out_file = open(output_dir_name + os.path.basename(filepath).replace(".xml", ".col"), "w")
                    cat_cols = CATToColumns.CATToColumns(filepath)
                    out_file.write(cat_cols.parseCAT())
                    out_file.close()
        ...
        cat_cols = CATToColumns.CATToColumns(sys.argv[1])
        out_file.write(cat_cols.parseCAT())
        out_file.close()
