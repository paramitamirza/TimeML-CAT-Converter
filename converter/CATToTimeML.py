...
class CATToTimeML:
    ...
    def generateTimeML(self):
        ...
        for (tid, tok, sid) in self.arr_tokens:
            ...
            if tid in self.event_starts:
                text += " <EVENT eid=\"e" + self.event_starts[tid][0] + "\" class=\"" + self.event_starts[tid][1] + "\">" + tok
    ...
    def parseCAT(self):
        ...
        for event in events:
            ...
            self.event_starts[start_id] = (event.get("id"), event.get("class"))
